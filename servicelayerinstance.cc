...
bool ServiceLayerInstance::Chirp(const std::string &username, const std::string &text, const std::string &parentid) {
	...
	kvstore->Put("chirp" + next_chirp_ID, chirp_string_value);//Add chirp::Chirp to database
	...
	if (parentid != "") {
		...
		kvstore->Put("reply"+ parentid, reply_chirp_string_value); //Add reply<ID> to backend
	}
	return true;
}
...
std::vector <chirp::Chirp> ServiceLayerInstance::Read(const std::string &chirpid) {
	...
	//Sort by timestamp
	std::sort(chirp_to_send.begin(), chirp_to_send.end(), [](chirp::Chirp &a, chirp::Chirp &b){
...
std::vector <chirp::Chirp> ServiceLayerInstance::Monitor(const std::string &username) {
		...
		for (int i = 0; i < followers.username_size(); i++) {
			...
			if(allfollowers.size() > 0) {
				...
				for(int j = 0; j < user_followers.chirps_size(); j++) {
					...
					if (it == chirpsent.end()) {
						if (user_followers.chirps(j).timestamp().useconds() > microseconds_since_epoch) {
...
void ServiceLayerInstance::CopyChirp(chirp::Chirp* chirp_, const chirp::Chirp &reply_) {
...
chirp::ChirpReplies ServiceLayerInstance::ConvertToChirpReplies(std::string byte) {
	chirp::ChirpReplies replies;
...
void ServiceLayerInstance::SetChirpReply(chirp::Chirp* chirp, chirp::ChirpReply* response) {
